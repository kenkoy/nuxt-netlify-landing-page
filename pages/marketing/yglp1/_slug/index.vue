...
<script>
export default {
  layout: 'marketing/yglp1', // Change to actual layout
  async asyncData ({ params, error }) {
    try {
      const markDownData = await import('~/assets/content/landing-page/marketing/yglp1/' + params.slug + '.md') // Change to actual slug
      return { markDownData }
    } catch (e) {
      error(e)
    }
  ...
  methods: {
    emitData (data) {
      this.$root.$emit('yglp1-data', { // Change to actual page name
